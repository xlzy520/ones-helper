  ...
  onesConfigService.getOtherConfig().then((res) => {
    setInterval(() => {
      ...
      if (false) {
        ...
        if (button && crumb) {
          const getAPi = () => {
            return fetch('https://marsdev-ci.myones.net/view/BUILD_PACKAGE/job/build-image/buildHistory/ajax', {
              method: 'POST',
              headers: {
                'jenkins-crumb': crumb,
          ...
          // getAPi()
          const latestBuild = document.querySelector('.build-row-cell .pane.build-name .display-name')
          if (latestBuild) {
            if (!currentBuildHref) {
