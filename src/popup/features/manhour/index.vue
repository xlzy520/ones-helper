    ...
    >
      ...
      >
        ...
        <n-form-item label="投入时长" path="hours">
          <n-input-number
            v-model:value="recordFormData.hours"
            min="0.1"
            :step="0.5"
            placeholder="请输入输入数字"
          >
...
const setOptions = (tableData: any) => {
  ...
  filterOptions.sprintOptions = Array.from(sprintSet).map((value) => ({
    label: value[1],
    value: value[0],
  }));
  filterOptions.statusOptions = Array.from(statusSet).map((value) => ({
    label: value[1],
    value: value[0],
  }));
};
...
const renderModalTitle = () => {
  return (
    <NEllipsis style="max-width: 460px;" tooltip={{ placement: 'bottom' }}>
      <NTag type="info" size="small" round={true} bordered={false}>
        #{selectTableRow.number}
      </NTag>
      ...
      return (
        <NTooltip
          ...
          v-slots={{
            trigger: () => {
              return (
                <div className="task-number cursor-pointer" onClick={handleClick}>
                  #{row.number}
                </div>
              );
            },
...
const filteredTableData = computed(() => {
  ...
  return tableData.value.filter((data: any) => {
    ...
    return (
      name.toLowerCase().includes(lowCaseFilterKey) &&
