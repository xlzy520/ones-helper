  ...
  <div class="py-5">
    ...
    >
      <n-grid>
        ...
        <n-form-item-grid-item :span="22" label="添加个数">
          <n-input-number v-model:value="formValue.count" :show-button="false" :placeholder="50" />
        </n-form-item-grid-item>
        <n-form-item-grid-item :span="22" label="单次请求个数">
          <n-input-number v-model:value="formValue.reqCountOneTime" :show-button="false" />
        </n-form-item-grid-item>
        <n-form-item-grid-item :span="22" label="起始下标">
          <n-input-number v-model:value="formValue.start" :show-button="false" />
        </n-form-item-grid-item>
        <n-form-item-grid-item
          :span="99"
          label="请求数据参数"
    ...
    <n-alert title="建议空闲时间使用此功能" type="info"> </n-alert>
  </div>
</template>
...
async function onSubmitClick(shouldClose = true) {
  const { url, body, count, reqCountOneTime, start } = formValue;
  sendMessage({
    type: 'batchRequest',
    data: {
      url,
