  ...
  <div>
    <div class="layout-slide">
      <div class="flex w-full">
        ...
        >
          <template #trigger>
            <n-button type="warning"> 恢复默认配置 </n-button>
          </template>
          恢复从项目配置读取的配置
        </n-popconfirm>
        <n-popconfirm
    ...
    <div class="h-[300px] overflow-auto pb-4">
      <div
        v-for="item in filterConfigFields"
        :key="item.key"
        class="layout-items-center py-1 mb-2 flex items-start flex-col"
      >
        <n-card size="small">
          <template #header>
            <div class="flex">
          ...
          <div class="flex-1 w-full">
            ...
            <n-input v-else v-model:value="item.value" placeholder="" class="flex-1" clearable>
              />
            </n-input>
          </div>
        </n-card>
...
const filterConfigFields = computed(() => {
  if (!filterKey.value) return configFields.value;

  return configFields.value.filter((v) =>
    v.key.toLowerCase().includes(filterKey.value.toLowerCase())
  );
});
